...
int set_env_variable(char * cmdline);
char * get_env_variable(char * cmdline);
...
int main()
{
	...
	while ((cmdline = read_cmd(prompt, stdin)) != NULL) {
		if (set_env_variable(cmdline) == 1) {
			continue;
		}
		if ((temp = get_env_variable(cmdline)) != NULL) {
			printf("%s\n", temp);
			continue;
		}
...
char * get_env_variable(char * cmdline) {
	char ** argument_list = tokenize(cmdline);
	if (strcmp(argument_list[0] , "echo") == 0) {
		char * variable = NULL;
		if ((variable = strchr(cmdline, '$')) != NULL) {
			variable++;
